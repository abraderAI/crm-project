...
|- **Orphaned Ticket Claiming** — When a user registers via Clerk and their email matches existing orphaned tickets, a dismissible banner appears on their first visit to `/support`: "We found N ticket(s) created before you registered." Users select tickets via checkboxes and click "Claim Selected" to transfer ownership (`POST /v1/support/claim-tickets`). Claiming sets `author_id` to the new user, nullifies `contact_email` (PII cleared), and posts a system event entry ("Ticket claimed by {name}"). The banner can be dismissed and persists via `localStorage`.
...
|| `/support` | Support tickets — tier-aware list with open/pending/resolved stats strip; **Unregistered** / **Registered** badges on tickets without org. Unclaimed-ticket banner for newly registered users whose email matches orphaned tickets (claim flow with checkbox selection). **Open** launches the multi-entry ticket editor: chronological timeline of customer messages, agent replies, drafts, internal notes, and system events; initial ticket body auto-saved as first entry; per-entry publish / edit / DEFT-visibility controls; status transitions (tiers 4–6) |
|| `/support/tickets/new` | New ticket creation — DEFT members (tier 4+) see an additional "Customer email" field to assign tickets by email; unregistered emails create orphaned tickets linked on registration |
...
The `global-support` and `global-leads` spaces are served under `/v1/global-spaces/{space}/threads`. Support ticket endpoints enforce **three-tier visibility scoping** server-side: individual users see only own/assigned tickets; org members see all org tickets; DEFT/admin users see all tickets. The `mine` and `org_id` query params further narrow results within the caller's permitted scope.
...
|| `PATCH` | `/v1/global-spaces/{space}/threads/{slug}` | Update thread body, status, and/or assignee; `assigned_to` must be a DEFT org member (validated server-side); auto-transitions status `open` ↔ `assigned`; revision recorded |
...
|| `GET` | `/v1/support/deft-members` | List active DEFT org members with display name and email (DEFT-only) |
