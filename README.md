...
- **Community Forum** — Global public forum (no auth required to read) with threaded posts, nested replies, up-voting, FTS5-indexed content, and full admin moderation tools: pin, lock, hide, and per-post management. Forum posts are seeded with representative content on first boot.
...
- **Email-based Ticket Assignment** — DEFT agents can create support tickets on behalf of unregistered or registered contacts by entering an email address. If the email matches a registered user, the ticket is attributed to them (with their org badge); if unregistered, the ticket is held as orphaned until the contact registers and claims it. The ticket list and detail page both surface the contact email as the requestor. Ticket list supports email-based search. Case-insensitive email lookup ensures correct attribution regardless of casing.
